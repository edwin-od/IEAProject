...
for r in range(1, 61):    
    ...
    blackAndWhiteImage = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 4)
    ...
    if black_pix <=22: 
        ...
        old+=1
    if r==30:
       print( 'efficiency',((young-old)/young)*100,'%')
       young=0
       old=0
    if r==61:
        print( 'efficiency',((old-young)/old)*100,'%')
    print('Y', str(r), 'black:', black_pix, '%','young',young,'old',old)
    sumofallpercentages+=black_pix
...
print('average percentage', sumofallpercentages/60)
